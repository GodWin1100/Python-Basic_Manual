...
print('\nit\'s a beautiful day')  #for not making python confuse which quotation is a statement we use back slash before it
...
print(state1,state2) #this doesnt add or concatenate the variable it just prints
...
print(state.find('M'))  #if letter are repeated in string then it will print the index number where it occured for the first time
print(state.find('s'),'\n')  #if character is not in string then it will return -1 (print)

restate=state.replace('MagiC','Dark Magic will')  #it replaces the particular character at index with some other string
